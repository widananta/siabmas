<!-- Modal -->
<div class="modal fade" id="change_pengampu_{{$d->id}}" tabindex="-1" role="dialog" aria-labelledby="editModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form method="post" action="/dashboard/pengampu/update" autocomplete="off">
                @csrf
                ...
                <div class="modal-body">
                    <div class="form-group{{ $errors->has('tahun') ? ' has-danger' : '' }}">
                        ...
                        <input type="text" name="tahun" id="input-name" class="form-control form-control-alternative{{ $errors->has('tahun') ? ' is-invalid' : '' }}" placeholder="{{ __('Nama') }}" value="{{$d->tahun}}" required readonly autofocus>
                        @if ($errors->has('tahun'))
                    ...
                    <div class="form-group{{ $errors->has('mata_kuliah') ? ' has-danger' : '' }}">
                        ...
                        <select name="mata_kuliah" required class="form-control" class="form-control form-control-alternative{{ $errors->has('mata_kuliah') ? ' is-invalid' : '' }}" value="l">
                            ...
                            @foreach ($matkul as $optmatkul)
                                <option 
                                @if ($d->matkul_id==$optmatkul->id)
                                    selected
                                @endif
                                value="{{$optmatkul->id}}">{{$optmatkul->nama_matkul}}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group{{ $errors->has('dosen') ? ' has-danger' : '' }}">
                        ...
                        <select name="dosen" required class="form-control" class="form-control form-control-alternative{{ $errors->has('dosen') ? ' is-invalid' : '' }}" value="l">
                            ...
                            @foreach ($dosen as $optdosen)
                                <option 
                                @if ($d->user_id==$optdosen->id)
                                    selected
                                @endif
                                value="{{$optdosen->id}}">{{$optdosen->name}}</option>
                            @endforeach
                        </select>
                        @if ($errors->has('dosen'))
                            <span class="invalid-feedback" role="alert">
